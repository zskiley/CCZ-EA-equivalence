...
void BuildEAHyperplaneSubsetByWalsh(const GraphData& F, std::size_t threshold,
  ...
  *hyperplanes_partition = OrderedPartition();
  ...
  const int64_t total_i = static_cast<int64_t>(total_points);
  auto counts_from_walsh = [total_i](int64_t w64, int* c0, int* c1) {
    const int64_t v0 = total_i + w64;
    const int64_t v1 = total_i - w64;
    *c0 = static_cast<int>(v0 / 2);
    *c1 = static_cast<int>(v1 / 2);
  };

  auto color_index = [](int count, bool x_only) -> std::size_t {
    return (static_cast<std::size_t>(count) << 1) | (x_only ? 1u : 0u);
  };

  std::vector<uint32_t> hist((total_points + 1u) * 2u, 0u);
  for (std::size_t normal = 1; normal < normals; ++normal) {
    int c0 = 0;
    int c1 = 0;
    counts_from_walsh(walsh[normal], &c0, &c1);
    const bool x_only = ((normal >> n) == 0u);
    const std::size_t color0 = color_index(c0, x_only);
    const std::size_t color1 = color_index(c1, x_only);
    ++hist[color0];
    ++hist[color1];
  }
  ...
  std::vector<uint8_t> keep;
  std::size_t keep_total = 0;
  (void)SelectSmallestColorClasses(hist, threshold, &keep, &keep_total);
  ...
  for (std::size_t normal = 1; normal < normals; ++normal) {
    ...
    if (keep[color0] != 0u) {
      const uint32_t plane_index = static_cast<uint32_t>(planes->size());
      planes->push_back(Hyperplane{static_cast<uint32_t>(normal), 0u});
      const int32_t cell_id = color_to_cell[color0];
      cells[static_cast<std::size_t>(cell_id)].push_back(plane_index);
    }
    if (keep[color1] != 0u) {
      const uint32_t plane_index = static_cast<uint32_t>(planes->size());
      planes->push_back(Hyperplane{static_cast<uint32_t>(normal), 1u});
      const int32_t cell_id = color_to_cell[color1];
      cells[static_cast<std::size_t>(cell_id)].push_back(plane_index);
    }
  }
